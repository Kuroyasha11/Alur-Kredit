    ...
    <div class="col-lg-15">
        <div class="row">
            <div class="d-flex justify-content-start">
                ...
                <form action="/manajer/{{ $request->id }}" method="post">
                    @method('put')
                    @csrf
                    <input type="hidden" name="submitanalis" value="0">
                    <button <abbr title="Batalkan" class="btn btn-danger my-2 mx-2" onclick="return confirm('Are you sure?')">
                        <i class="bi bi-x-lg"></i>
                    </button>
                </form>
                @if (auth()->user()->is_komite1)
                    @if ($request->analyst->plafon <= 50000000)
                        <form action="/manajer/{{ $request->id }}" method="post">
                            @method('put')
                            @csrf
                            <input type="hidden" name="selesaimanajer" value="1">
                    ...
                    @elseif ($request->analyst->plafon > 50000000)
                        <form action="/manajer/{{ $request->id }}" method="post">
                            @method('put')
                            @csrf
                            <input type="hidden" name="selesaimanajer" value="1">
                            <input type="hidden" name="selesaidirut" value="0">
                            <button <abbr title="Submit" class="btn btn-success my-2 mx-2"
                                onclick="return confirm('Are you sure?')">
                                <i class="bi bi-check-lg"></i>
                            </button>
            ...
            <div class="table-responsive col-lg-15 mb-5">
                ...
                <div class="d-flex justify-content-end">
                    <a href="/manajer" class="btn btn-warning my-2 mx-2">Kembali</a>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="/manajer/{{ $request->analyst->id }}" method="post">
                        @method('put')
                        @csrf
